...
public class ContentsController {
   ...
   public ModelAndView postingCommunity(CommunityBoardVO cvo,FileVO vo){
	   ...
	   for (int i = 0; i < list.size(); i++) {
		   ...
		   if (!fileName.equals("")) {
			   try {
				   File f = new File(commuPath+cvo.getBoardNo()+"_"+fileName);
	...
	public ModelAndView writeFormInCommunity(HttpServletRequest request){
	...
	@RequestMapping(value="my_updateCommunityForm.do")
	...
	/* 커뮤니티 게시판 게시물 수정 */
	@RequestMapping(value = "my_updateCommunity.do")
   ...
   @RequestMapping(value = "my_getCommunityByNo.do")
   public ModelAndView getCommunityByNo(HttpServletRequest request, int boardNo){
	   ModelAndView mv = new ModelAndView("my_community_info");
	   BoardVO vo = (CommunityBoardVO)communityBoardService.getCommunityByNo(boardNo);
	   List<ReplyVO> list = communityBoardService.getReplyListByNo(boardNo);
