...
$(function(){
//운동부위 값을 받아오면서 value에 맞게 selected를 해준다.
...
<div class="col-md-6" >
<form id="exerciseUpdateForm" method="post" action="${initParam.root }admin_updateExerciseByAdmin.do" enctype="multipart/form-data" class="form-horizontal">
	<input type="hidden" name="momentorMemberVO.memberId" value="${sessionScope.pnvo.momentorMemberVO.memberId }">
	<input type="hidden" id="exerciseName" name="exerciseName" value="${info.exerciseVO.exerciseName }" required="required" size="40" readonly="readonly">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" name="boardTitle" value="${info.boardTitle }" required="required" size="40" class="form-control">
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			${info.exerciseVO.exerciseName }
		</div>
	</div>

<c:set var="urlPath" value="${requestScope.URLVideo}"/>
	<div class="form-group">
	  <label for="urlPath" class="col-sm-2 control-label"> URL : </label>
		<div class="col-sm-10">
			<input type="text" id="urlPath" name="urlPath" placeholder="운동 영상 주소를 넣어주세요(있을시만)"  size="40" class="form-control" value="${urlPath.URLPATH }">
		</div>
	</div>
